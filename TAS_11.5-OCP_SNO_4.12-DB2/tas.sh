...
echo "Creating secret object with DB2 certificate."
...
while \
[ "$TASCR" != "TRIRIGA Application Suite is ready." ]; \
do TASCR=$(oc get Tririga my-tririga -o jsonpath='{.status.conditions[0].message}{"\n"}'); echo "Wait 2+ hours!..." $TASCR; sleep 40; done
